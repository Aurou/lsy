...
void add_driver(struct series *series) {
    ...
    if (driver_exists_in_race(race, driver_name)) {
        printf("ERROR: Driver with name %s "
               "already exists in race.\n", driver_name);
        return;
    }
...
void print_series(struct series *series) {
    ...
    while (current_race != NULL) {
        ...
        if (current_race->status == ACTIVE) {
            struct driver_rank *ranks = calculate_driver_ranks(current_race);
            print_track(current_race->track, ranks);
            free_driver_ranks(ranks);
        } else {
            print_track(current_race->track, NULL);
        }
...
struct driver_rank *calculate_driver_ranks(struct race *race) {
    ...
    for (int i = len; i >= 1; i--) {
        ...
        while (current_driver != NULL) {
            struct driver_rank *new_rank = malloc(sizeof(struct driver_rank));
            strcpy(new_rank->name, current_driver->name);
            new_rank->rank = rank;
            new_rank->next = NULL;
...
int find_driver_rank(struct driver_rank *ranks, char *name) {
    struct driver_rank *current = ranks;
    while (current != NULL) {
        if (strcmp(current->name, name) == 0) {
            return current->rank;
        }
...
void free_driver_ranks(struct driver_rank *ranks) {
    struct driver_rank *current = ranks;
    while (current != NULL) {
        struct driver_rank *temp = current;
        current = current->next;
        free(temp);
    }
}
    ...
    while (track != NULL) {
        ...
        if (track->drivers != NULL) {
            ...
            while (current_driver != NULL) {
                if (ranks != NULL) {
                    // ACTIVE 状态下使用全局排名
                    int global_rank = find_driver_rank(ranks,
                        current_driver->name);
                    printf("        %d. %s\n", global_rank,
                        current_driver->name);
                } else {
...
void play_single_turn(struct race *race) {
    ...
    for (int i = len; i >= 1; i--) {
        ...
        while (current_driver != NULL) {
            ...
            remove_driver_from_list(&current_track->drivers,
                current_driver->name);
        ...
        switch (cmd) {
            ...
            case 'i': {
                ...
                if (sub_cmd == 't') {
                    ...
                    if (driver_exists_in_race(race, driver_name)) {
                        printf("ERROR: Driver with name "
                               "%s already exists in race.\n", driver_name);
                        break;
                    }
                    ...
                    struct driver *new_driver =
                        create_driver(driver_name, speed);
            ...
            case 'c': {
                ...
                struct race *new_race =
                    create_race(new_name, existing_race->location);
                struct track_piece *current_track = existing_race->track;
                while (current_track != NULL) {
                    struct track_piece *copied_track =
                        create_track(current_track->type);
            ...
            case 'v': {
                ...
                while (current != NULL && current->next != NULL) {
                    if ((current->type == RIGHT_TURN &&
                        current->next->type == RIGHT_TURN) ||
                        (current->type == LEFT_TURN &&
                            current->next->type == LEFT_TURN)) {
                        struct track_piece *straight = create_track(STRAIGHT);
                        straight->next = current->next;
                        current->next = straight;
                    }
                    if (current->type == JUMP &&
                        current->next->type != STRAIGHT) {
                        struct track_piece *straight = create_track(STRAIGHT);
                        straight->next = current->next;
                        current->next = straight;
                    }
            ...
            case 'm': {
                ...
                if (driver_exists_in_race(new_race, driver_name)) {
                    printf("ERROR: Driver %s already "
                           "in race %s.\n", driver_name, new_race_name);
                    break;
                }
                if (driver_count(new_race->drivers) >= MAX_DRIVERS) {
                    printf("ERROR: Race %s already at "
                           "maximum driver limit.\n", new_race_name);
                    break;
                }

                struct driver *moved_driver =
                    remove_driver_from_list(&current_race->drivers, driver_name);
            ...
            case 'r': {
                ...
                while (current_race != NULL) {
                    if (current_race->status == NOT_STARTED) {
                        ...
                        while (current_track != NULL) {
                            struct driver *removed =
                                remove_driver_from_list(&current_track->drivers, driver_name);
                            if (removed != NULL) {
                                free(removed);
                                found = 1;
                            }
                ...
                if (!found) {
                    ...
                    printf("Driver %s removed from the "
                           "racing series!\n", driver_name);
                }
                break;
            }
