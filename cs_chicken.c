...
//
// Description: COMP1511 26T1 Assignment 1 - CS Chicken Game (Stage 1-3 Complete, 滚动逻辑完美修复)
// Provided Libraries
#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    while (scanf(" %c", &gameplay_cmd) != EOF) {
        ...
        // ==============================================
        int prev_player_row = player_row; // 移动前的行号
        int prev_player_col = player_col; // 移动前的列号
        int move_dir = -1;
        int is_rest = 0;
        int moved_up = 0;
        int attempted_up_from_row0 = 0;
        int used_tunnel = 0; // [关键新增] 标记是否使用了隧道传送
