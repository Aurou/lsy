...
//
// Description: COMP1511 26T1 Assignment 1 - CS Chicken Game (Stage 1-3 Complete, 隧道滚动逻辑最终修复)
// Provided Libraries
#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    while (scanf(" %c", &gameplay_cmd) != EOF) {
        ...
        // ==============================================
        int prev_player_row = player_row; // 移动前的行号，用于记录
        ...
        if (game_mode == SCROLLING_MODE) {
            ...
            // 2. 移动成功
            // 3. 不是从第0行向上的特殊情况
            // 4. **传送后的最终位置**在0-6行（而不是移动前的位置）
            if (moved_up && move_successful && !attempted_up_from_row0 && player_row <= 6) {
                scroll_board(board, original_coins, tunnel_id_map);
                scroll_triggered = 1;
                // 所有行下移，玩家位置同步下移
                player_row += 1;
