...
int check_car_collision(struct tile board[ROWS][COLS],
...
int main(void) {
    ...
    print_board(board, player_row, player_col, curr_score, target_score,
        tunnel_id_map);
    ...
    while (scanf(" %c", &cmd) != EOF) {
        ...
        // Stage 1.4: CAR Command
            ...
            if (board[row][col].entity != ROAD && board[row][col].entity !=
                HEADLIGHTS) {
            ...
            if (is_position_valid(row, headlight_col)) {
                ...
                int is_car_present = (headlight_tile == CAR_FACING_LEFT ||
                    headlight_tile == CAR_FACING_RIGHT);
                if (!is_car_present) {
        ...
        // Stage 3.4: Wombat Tunnel Command
            ...
            if (sub_cmd == 't') {
                ...
                if (!is_position_valid(row1, col1) ||
                    ...
                    printf("Invalid location: wombat couldn't dig a tunnel "
                           "here!\n");
                    continue;
                }
                // Check positions are not occupied
                if (is_tile_occupied(board, player_row, player_col,
					row1, col1) || is_tile_occupied(board,
					player_row, player_col, row2, col2)) {
                    printf("Invalid location: wombat couldn't dig a tunnel "
                           "here!\n");
                    continue;
                }
    ...
    // ==============================================
    print_board(board, player_row, player_col, curr_score,
    ...
    while (scanf(" %c", &gameplay_cmd) != EOF) {
        ...
        if (gameplay_cmd == 'p') {
            print_game_statistics(turns_taken, step_count,
                coins_collected, curr_score);
            continue;
        }
            ...
            if (attempted_up_from_row0 && game_mode == SCROLLING_MODE) {
                if (board[9][player_col].entity != TREE) {
                    ...
                    if (board[player_row][player_col].entity == WOMBAT_TUNNEL) {
                        ...
                        find_tunnel_exit(tid, player_row, player_col,
...
void find_tunnel_exit(
    ...
    *exit_col = -1;
    for (int row = 0; row < ROWS; row++) {
        for (int col = 0; col < COLS; col++) {
            if (tunnel_id_map[row][col] == tunnel_id && !(row == curr_row
                && col == curr_col)) {
                *exit_row = row;
                *exit_col = col;
                return;
    ...
    for (int row = 0; row < ROWS; row++) {
        for (int col = 0; col < COLS; col++) {
            ...
            if (row == player_row && col == player_col) {
                ...
                enum entity e = board[row][col].entity;
                if (e == HEADLIGHTS || e == CAR_FACING_LEFT ||
                    e == CAR_FACING_RIGHT) {
                    printf("0_0");
                } else {
                    printf("^_^");
