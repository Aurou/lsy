...
int main(void) {
    ...
    while (scanf(" %c", &cmd) != EOF && cmd != '~') {
        ...
        // Stage 1.4: CAR Command
            ...
            if (is_position_valid(row, headlight_col)) {
                int is_opposite_car = 0;
                if (dir == 'r' && board[row][headlight_col].entity == CAR_FACING_LEFT) {
                    is_opposite_car = 1;
                } else if (dir == 'l' && board[row][headlight_col].entity == CAR_FACING_RIGHT) {
                    is_opposite_car = 1;
                }
                if (!is_opposite_car) {
    ...
    while (scanf(" %c", &gameplay_cmd) != EOF) {
        ...
        if (!is_rest) {
            // Stage 2.2: Validate base movement
            if (is_move_valid(board, new_row, new_col)) {
                ...
                // Stage 3.4: Process Wombat Tunnel Teleport
                if (board[player_row][player_col].entity == WOMBAT_TUNNEL) {
                    ...
                    // Calculate landing position (same direction as movement)
                    ...
                    if (!is_position_valid(land_row, land_col) || board[land_row][land_col].entity == TREE) {
                        // Invalid landing: revert movement
                        move_successful = 0;
                        player_row -= dr[move_dir];
                        player_col -= dc[move_dir];
                        step_count--;
                    } else {
                        // Valid teleport: move to landing position
                        player_row = land_row;
                        player_col = land_col;
                    }
                }

                // Stage 2.3: Process coin collection
        ...
        // Stage 3.1: Check car collision after player move
        ...
        // ==============================================
        // Stage 3.2: Car Turn (Driving & Scrolling Modes)
        // ==============================================
        if (game_mode == DRIVING_MODE || game_mode == SCROLLING_MODE) {
            process_car_turn(board);

            // Check collision after car movement
            if (check_car_collision(board, player_row, player_col)) {
                trigger_game_loss(
                    board, player_row, player_col, curr_score, target_score,
                    turns_taken, step_count, coins_collected, tunnel_id_map
        ...
        // ==============================================
        // Stage 3.3: Other Mechanics Turn (Scrolling Mode)
        // ==============================================
        if (game_mode == SCROLLING_MODE) {
            int scroll_triggered = 0;

            // Handle player attempting to move up from row 0
            if (attempted_up_from_row0) {
                // Check if the row to be scrolled (current row9) has a tree in target column
                if (board[9][player_col].entity != TREE) {
                    // Execute scroll
                    scroll_board(board, original_coins, tunnel_id_map);
                    scroll_triggered = 1;
                    // Move player to the new top row
                    player_row = 0;
                    step_count++;

                    // Process coin collection on new position
                    if (board[player_row][player_col].entity == COIN) {
                        board[player_row][player_col].entity = EMPTY;
                        curr_score += 5;
                        coins_collected++;
            ...
            // Handle normal scroll trigger (moved up in rows 0-6)
            else if (moved_up && move_successful && player_row <= 6) {
                scroll_board(board, original_coins, tunnel_id_map);
                scroll_triggered = 1;
                // Shift player position down as all rows shifted down
                player_row += 1;
            }

            // Post-scroll checks
            if (scroll_triggered) {
                // Check collision after scroll
                if (check_car_collision(board, player_row, player_col)) {
                    trigger_game_loss(
                        board, player_row, player_col, curr_score, target_score,
                        turns_taken, step_count, coins_collected, tunnel_id_map
        ...
        // Print updated board after full round
        print_board(board, player_row, player_col, curr_score, target_score, tunnel_id_map);
    }
...
void process_car_turn(struct tile board[ROWS][COLS]) {
    ...
    for (int i = 0; i < car_count; i++) {
        ...
        // Place headlight if valid and not a car
        if (is_position_valid(row, headlight_col)) {
...
void print_board(
    ...
    for (int row = 0; row < ROWS; row++) {
        for (int col = 0; col < COLS; col++) {
            printf("|");
            if (row == player_row && col == player_col) {
                // Stage 3.1: Shocked chicken if on headlights
                enum entity e = board[row][col].entity;
